...
═══════════════════════════════════════════════════════════════════════════
- **STATUS:** 8 chapters done, ALL in **CBSE Class 8 / arithmetic** (the whole `explain/class8/arithmetic/` folder is now rich). Everything else is still on the OLD animation (they only have the shared slow-narration `rishi-explain-slow.js` + math keyboard + I-Don't-Understand button — NOT the v3 story scenes).
- **WHAT'S LEFT (not yet rich):** CBSE Class 8 algebra (factorisation, algebraic-expressions-identities, linear-equations, introduction-to-graphs), data-handling (chance-probability, frequency-distribution), geometry (understanding-quadrilaterals, practical-geometry, visualising-solid-shapes), mensuration (area-plane-figures, surface-area-volume); then **all of CBSE class6/7/9 and all ICSE class6/7/8/9**. ~132 explain pages remain.
