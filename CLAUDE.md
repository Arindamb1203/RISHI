...
- **Playing with Numbers (ch16)** — `PLAYING WITH NUMBERS family` (6 concepts): showcase `genForm2` (a 2-digit number is 10a+b, shown as ten-blocks + unit dots); `genForm3`, `divRule` (one concept covers the 2/3/5/9/11 rules via m), `reverseSum` (ab+ba = 11(a+b)), `reverseDiff` (ab−ba = 9(a−b)), `findDigit` (5A7÷9 → A). **This page is NON-STANDARD** (single-line `showQ`/`startAnim`/`replayAnim`, `var` split across two lines, `buildSession` already pins `QB[0]`) — wiring adapted accordingly; old `startAnim` left as harmless dead code (its `getAnimPlay(q.anim)` is never called).
- **Direct & Inverse Proportions (ch13)** — `PROPORTION family` (6 concepts): showcase `directProp` (unitary method, "more pens cost proportionally more"; `perStr`/`ansStr` carry the worked numbers so one concept fits all 7 direct questions); `inverseProp` (product constant, "more workers fewer days"; `prodStr`/`k`/`ansStr`); `directRule`/`inverseRule` (x doubles→y doubles / x triples→y⅓), `directFormula` (x/y=k), `inverseFormula` (xy=k). Generic string-driven so each question stays correct. Standard template; `startAnim` removed cleanly.
- **Story of Numbers (ch18)** — `STORY OF NUMBERS family` (14 concepts, HISTORY chapter — illustrative not math): showcase `tallyBones` (notches carved on the Lebombo/Ishango bone); `bodyCount`,`egyptian`,`roman`,`romanConvert`,`babylonian`,`mayan` (dots/bars/shell-zero),`chineseRod`,`baseConcept`,`baseConvert` (one concept for base-5 AND base-2 via m),`hindu`,`brahmagupta`,`spread`,`compareSystem`. Ancient symbols drawn with simple shapes/letters (no emoji — shell shown as a small rounded rect). No skin. Owner chose to do this history chapter rich anyway.
- **Rich chapters so far: Squares · Cubes · Exponents · Comparing Quantities · Rational Numbers · Playing with Numbers · Direct & Inverse Proportions · Story of Numbers (8).** Engine `rishi-animate.js` internal `version:3` label is stale/cosmetic (cache-bust is the `?v=` query, currently v4) — not bumped, harmless.

═══════════════════════════════════════════════════════════════════════════
RICH-ANIMATION ROLLOUT — HANDOFF / CONTINUE-LATER NOTE (paused after Story of Numbers, 12 Jun 2026)
═══════════════════════════════════════════════════════════════════════════
- **STATUS:** 9 chapters done — all 8 of **CBSE Class 8 / arithmetic** + the 1st algebra one, **Algebraic Expressions & Identities** (`ALGEBRA family`: showcase `sqIdentityPlus` = the (a+b)² area-model square-cut-into-4; plus `sqIdentityMinus`,`diffSquares`,`xaxb`,`squareTrick` (102²/98² via identity, one concept both signs),`foil`,`combineTerms`,`subtractExpr`,`terms`. 10 questions. Page is a LARGE custom build (1203 lines, multi-line funcs, `var` split, `buildSession` pinned QB[0]; had a duplicate `confirmShown=false;`). Maths spot-checked exact.). Everything else is still on the OLD animation (shared slow-narration + math keyboard + I-Don't-Understand only — NOT v3 story scenes).
- **WHAT'S LEFT (not yet rich):** CBSE Class 8 algebra (factorisation, linear-equations, introduction-to-graphs), data-handling (chance-probability, frequency-distribution), geometry (understanding-quadrilaterals, practical-geometry, visualising-solid-shapes), mensuration (area-plane-figures, surface-area-volume); then **all of CBSE class6/7/9 and all ICSE class6/7/8/9**. ~131 explain pages remain.
- **THE RECIPE (repeat per chapter — proven 8× this session):**
  1. `grep` the page's QB: map each `id`→`anim`→question; note if NON-STANDARD (e.g. playing-with-numbers had single-line funcs + `var` split; check the `var atTimers`/`var session` line and `buildSession`).
  2. Add a concept FAMILY to `rishi-animate.js` before the `/* skin picker */` block: each concept = `R.name={scene:function(m,sk){return {base:stage(...),phases:[{cap,ms,pause,say,frag},…3+]};}}`. Reuse existing helpers (`stage,T,RC,CI,PG,LN,answerBox,spark,chip,braceH,numLine,block,mBar,pctStrip,drawBox,cubeLayer,eTile,sayFrac,drawItem`). Add a `XXX_CONCEPTS={}` registry; if the chapter needs skins, add a `_SKINS` pool + extend `pickSkin` (pool-aware: CUBE/MONEY pools already wired; abstract concepts ignore skin).
  3. **Validate engine BEFORE touching the page:** `node --check public/rishi-animate.js` + a runtime loop that renders every concept/m (`/^<svg/`, has `rzStage`, ≥3 phases, every `say`+`frag` a string, no `undefined`, no `function` leak, has a shape) AND confirms earlier families still render.
  4. Wire the page — 6 edits: (a) add `<script src="/rishi-animate.js?v=4"></script>` after rishi-presence; (b) insert `buildScene`/`playScene`(with `_playGen` guard + min-dwell pacing)/`finishAnim` before the `var atTimers…/var session…` line; (c) `showQ`: add `buildScene(q);` after `confirmShown=false;`; (d) panel innerHTML → `q._scene.base`, header "Watch & Learn", drop the Play/Step buttons, `setTimeout(playScene,700)`; (e) `replayAnim` → re-`buildScene`+`q._scene.base`+`playScene`; (f) `buildSession` stable-sort to pin the showcase concept first. Then 15 QB edits: `anim:"xx",` → `concept:"…",m:{…},` (each `anim:"xx",` is a unique anchor; watch case e.g. `findA`).
  5. **Validate page:** 0 stale `getAnimSVG(q.anim)/q.anim/anim:"xx"` refs (a lone dead `startAnim` is OK if you left it), concept count == question count, inline scripts parse 0 errors (vm.Script per `<script>` without src), `audit_explain.py` shows the page REAL + button present.
  6. Update CLAUDE.md → `git add` (engine + page + CLAUDE.md) → commit → push → **verify live** (poll `rishi-ewh.pages.dev/rishi-animate.js` for the new concept name AND `rishion.in/explain/…` for it; deploy lags ~15-30s).
- **⚠️ VERIFY ALL CORRECTIONS MADE BEFORE THIS PAUSE:** this session shipped 8 chapters + the shared math-keyboard cascade + the I-Don't-Understand button fix + the slow-narration cascade, fast and back-to-back. **Before building more, re-check the 8 rich pages live on rishion.in** (Squares, Cubes, Exponents, Comparing Quantities, Rational Numbers, Playing with Numbers, Direct & Inverse Proportions, Story of Numbers): for each, click through several questions — confirm the animation plays slowly + narrated, the showcase shows first, "I Understand / I Don't Understand" appear on EVERY question, the math keyboard (incl ∛/√ chips) shows on the confirm question, and the maths in every scene is CORRECT (spot-check the numbers/answers per concept — these were authored by hand). Fix any that are off before cascading the pattern wider. NOTE: TTS still robotic until ElevenLabs credits are topped up (owner action) — judge pacing/visuals regardless.
